...
def get_dataset(src, name, distribution):
    data = src[src.airport == name].copy()
    del data['airport']
    data['date'] = pd.to_datetime(data.date)
    data['left'] = data.date - pd.DateOffset(days=1)
    data['right'] = data.date + pd.DateOffset(days=1)
    data = data.set_index(['date'])
    data.sort_index(inplace=True)
    if distribution == 'Smooth':
        window, order = 51, 3
        for key in STATISTICS:
            data[key] = savgol_filter(data[key], window, order)
...
def update_plot(attrname, old, new):
    ...
    src = get_dataset(data, cities[city]['airport'], distribution_select.value)
...
data = pd.read_csv(join(dirname(__file__), '2015_weather.csv'))
source = get_dataset(data, cities[city]['airport'], distribution)
